    ...
    <tbody id="contact-table">
      <?php
        if (file_exists('./dat/phone.dat')) {
          ...
          fwrite($fh, "{}");
          fclose($fh);
          $all_contacts = json_decode("{}", true);
        }
        $i=0;
        foreach($all_contacts as $id => $contact){
          $class = ( $i%2==0 ? 'even' : 'odd');
          echo sprintf('<tr rowid="%s" class="%s">', $id, $class);
          echo sprintf('<td><span data-name="fname" data-pk="%s" class="xedit fname">%s</span></td>', $id, $contact["fname"]);
          echo sprintf('<td><span data-name="lname" data-pk="%s" class="xedit lname">%s</span></td>', $id, $contact["lname"]);
          echo sprintf('<td><span data-name="phone" data-pk="%s" class="xedit phone">%s</span></td>', $id, $contact["phone"]);
